...
One tool, three names: `sshph`, `tarch`, `tailscale-ssh`. Install it on every device on
your tailnet (laptop, phone, another laptop, a server) and each one can reach any of the
others — a live, color-coded list of who's online, who answers SSH, and one keypress to
connect. No IPs, ports or usernames are hardcoded anywhere; there's nothing to edit when
your network changes, because nothing is stored except what Tailscale already knows.

```
◈ tailscale-ssh  ·  franc@example.com
...
Termux) and three thin wrapper commands — `sshph`, `tarch`, `tailscale-ssh` — to
`~/.local/bin` (`$PREFIX/bin` on Termux). All three run the exact same engine; the name
you type in just becomes the display brand.

First run walks new devices through setup automatically (or run `sshph setup` any
time):
...
Update any install later with `sshph update` (pulls the latest engine from GitHub).

## Using it

```
sshph                  # arrow-key list of every reachable device — pick one, hit enter
sshph pixel            # connect straight to a device by name (prefix match is fine)
sshph u0_a327@pixel    # ...or override the username
tarch ubt -- tmux attach   # any name works — pass a remote command after --
sshph list             # print the list once and exit (add --json for scripts)
sshph watch            # live dashboard, no connecting
sshph doctor           # diagnose "why can't I connect"
```

The same picker is what `sshph` and `tarch` show — a laptop and a phone see each other
...
- **What *is* remembered** (in `~/.config/tailscale-ssh/`) is just the small stuff
  ...
  `sshph forget <device>` clears one; `sshph config` shows the file.
...
install.sh         one-line installer — also creates the sshph/tarch/tailscale-ssh commands
...
in the repo or in the installer. For `install.sh`/`sshph update` to pull from a private
